...
fn validate_file(bytes: &Bytes) -> Option<FileType> {
    ...
    if bytes[0..JPG_HEADER.len()].eq(JPG_HEADER)
        && bytes[bytes.len() - JPG_FOOTER.len()..].eq(JPG_FOOTER)
    {
...
async fn upload(
    APIKey(key): APIKey,
    body: Bytes,
) -> miette::Result<(StatusCode, String), StatusCode> {
    if !API_KEY.eq(&key) {
...
) -> miette::Result<(HeaderMap, Vec<u8>), StatusCode> {
